...
public class GalleryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ActivityCompat.requestPermissions(this, GALLERY_PERMISSIONS, REQUEST_GALLERY_PERMISSIONS);
        ...
        textView.setText("Choose an image to classify between benign and malignant");

        // Here we load the assets: the only one is the pytorch model.
        try {
            module = Module.load(assetFilePath(this, "skin-rn50android512.ptl"));

            // Code below is for checking that load + tensor convert to float + normalization is the same here and in Android
            Bitmap assetImage = BitmapFactory.decodeFile(assetFilePath(this, "ASAN_0.png"));
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_ADD_FILE && resultCode == RESULT_OK && null != data) {
            ...
            textView.setText(String.format("Prediction: %s (%.2f %% beningno | %.2f %% maligno)", CLASSES[maxScoreIdx], scores[0] * 100, scores[1] * 100));
        }
    }
